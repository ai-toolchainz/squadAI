...
from squadai.memory.contextual.contextual_memory import ContextualMemory
from squadai.process import Process
from squadai.task import Task
from squadai.utilities import Logger, RPMController
...
def test_squad_config_conditional_requirement():
    with pytest.raises(ValueError):
        Squad(process=Process.sequential)
    ...
    try:
        squad = Squad(process=Process.sequential, config=config)
    except ValueError:
        pytest.fail("Unexpected ValidationError raised")

    assert [agent.role for agent in squad.agents] == [
        agent["role"] for agent in parsed_config["agents"]
    ]
    assert [task.description for task in squad.tasks] == [
        task["description"] for task in parsed_config["tasks"]
    ]


def test_squad_config_with_wrong_keys():
    ...
    with pytest.raises(ValueError):
        Squad(process=Process.sequential, config='{"wrong_key": "wrong_value"}')
    with pytest.raises(ValueError):
        Squad(process=Process.sequential, config=no_tasks_config)
    with pytest.raises(ValueError):
        Squad(process=Process.sequential, config=no_agents_config)
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_squad_with_delegating_agents():
    tasks = [
        Task(
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_squad_verbose_output(capsys):
    tasks = [
        Task(
    ...
    squad._logger = Logger(verbose_level=False)
    squad.kickoff()
    captured = capsys.readouterr()
    assert captured.out == ""
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_squad_verbose_levels_output(capsys):
    tasks = [
        Task(
    ...
    squad = Squad(agents=[researcher], tasks=tasks, process=Process.sequential, verbose=1)
    ...
    squad._logger = Logger(verbose_level=2)
    squad.kickoff()
    captured = capsys.readouterr()
    expected_strings = [
...
def test_api_calls_throttling(capsys):
    ...
    squad = Squad(agents=[agent], tasks=[task], max_rpm=2, verbose=2)
...
def test_squad_full_ouput():
    ...
    squad = Squad(agents=[agent], tasks=[task1, task2], full_output=True)
...
def test_agents_rpm_is_never_set_if_squad_max_RPM_is_not_set():
    agent = Agent(
        role="test role",
        goal="test goal",
        backstory="test backstory",
    ...
    Squad(agents=[agent], tasks=[task], verbose=2)
...
def test_dont_set_agents_step_callback_if_already_set():
    ...
    squad = Squad(
        ...
        process=Process.sequential,
        tasks=[list_ideas],
        step_callback=squad_callback,
    )

    with patch.object(Agent, "execute_task") as execute:
        execute.return_value = "ok"
        squad.kickoff()
        assert researcher_agent.step_callback is not squad_callback
        assert researcher_agent.step_callback is agent_callback


@pytest.mark.vcr(filter_headers=["authorization"])
def test_squad_function_calling_llm():
    from unittest.mock import patch
    ...
    with patch.object(llm.client, "create", wraps=llm.client.create) as private_mock:
        ...
        tasks = [essay]
        squad = Squad(agents=[agent1], tasks=tasks, function_calling_llm=llm)
        squad.kickoff()
...
def test_task_with_no_arguments():
    ...
    squad = Squad(agents=[researcher], tasks=[task])
...
def test_agents_do_not_get_delegation_tools_with_there_is_only_one_agent():
    ...
    squad = Squad(agents=[agent], tasks=[task])
...
def test_squad_inputs_interpolate_both_agents_and_tasks():
    agent = Agent(
    ...
    squad = Squad(agents=[agent], tasks=[task], inputs={"topic": "AI", "points": 5})

    assert squad.tasks[0].description == "Give me an analysis around AI."
    assert squad.tasks[0].expected_output == "5 bullet points about AI."
    assert squad.agents[0].role == "AI Researcher"
    assert squad.agents[0].goal == "Express hot takes on AI."
    assert squad.agents[0].backstory == "You have a lot of experience with AI."
...
def test_tools_with_custom_caching():
    ...
    squad = Squad(agents=[writer1, writer2], tasks=[task1, task2, task3, task4])

    with patch.object(
        CacheHandler, "add", wraps=squad._cache_handler.add
    ) as add_to_cache:
        with patch.object(
            CacheHandler, "read", wraps=squad._cache_handler.read
        ) as read_from_cache:
            result = squad.kickoff()
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_squad_log_file_output(tmp_path):
    ...
    squad = Squad(agents=[researcher], tasks=tasks, output_log_file=str(test_file))
    squad.kickoff()
...
def test_manager_agent():
    ...
    manager = Agent(
        role="Manager",
        goal="Manage the squad and ensure the tasks are completed efficiently.",
        backstory="You're an experienced manager, skilled in overseeing complex projects and guiding teams to success. Your role is to coordinate the efforts of the squad members, ensuring that each task is completed on time and to the highest standard.",
        allow_delegation=False,
    )
